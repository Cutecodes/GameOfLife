...
class Model(object):
    ...
    def __init__(self, num_of_rows=20, num_of_cols=20, ratio=0.1,filename=""):
        if filename=="":
            ...
            self.num_of_rows = int(file.readline())
            self.num_of_cols = int(file.readline())
            self.round = 0
            self.map = [[0 for i in range(self.num_of_cols)] for i in range(self.num_of_rows)]
            for i in range(self.num_of_rows):
                tempstr = file.readline()
                strlist = tempstr.split(' ')
                for j in range(self.num_of_cols):
                    self.map[i][j] = int(strlist[j])
            file.close()
